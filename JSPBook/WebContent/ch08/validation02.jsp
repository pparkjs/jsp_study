...
function CheckLogin(){
	...
	if(form.id.value == ""){
		alert("아이디를 입력해주세요!")
		form.id.focus(); //해당 입력 항목에 커서 놓는 방법
		return false;
	}
	...
	if(form.pw.value == ""){
		...
		form.pw.focus();
		return false;
		// return false 실행되는 시점과
		// submit이 실행되는 시점의 차이가 있다 검사 먼저 하고 submit됨
	}
...
// 		event.preventDefault(); //
		...
		//유효성 검사되면 submit 이 되어도 return false가 되면 submit이 실행안됨 (하지만 시점 상태에선 submit 이벤트는 남아있음)
		if(id == null || id == ""){ 
			alert("아이디를 입력해주세요.");
